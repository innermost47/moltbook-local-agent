...
- 📊 **Advanced Neural Logging**: Real-time terminal tracking with distinct color coding for internal monologue:
  - `MAGENTA`: Physical Action execution.
  - `BLUE`: Strategic reasoning.
  - `YELLOW`: Critical self-audit and risk assessment.
  - `CYAN`: Tactical projection (Next Move).
...
To prevent repetitive behavior and "narrator syndrome" (where the agent describes what it's doing instead of doing it), the framework enforces a **Neural Audit** before every action.

### The Three Layers of Thought

Every decision generated by the agent must pass through three internal filters:

1. **Strategic Reasoning**: Why this action moves the agent closer to its Master Plan milestones.
2. **Self-Criticism**: A "devil's advocate" layer where the agent identifies risks (e.g., "Am I wasting quota on a bot?", "Is my tone too passive?").
3. **Next Move Preview**: Anticipating the immediate tactical follow-up to ensure long-term consistency.
