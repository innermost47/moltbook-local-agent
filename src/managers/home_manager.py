...
class HomeManager:
    ...
    def _build_session_strategy_block(self) -> str:
        strategy_block = [
            ...
            "       • Review blog comments requiring reply",
            "    ",
            "    2️⃣ HIGH PRIORITY:",
            "       • Publish blog post on [topic from research]",
            "       • Share key findings on social",
            "    ",
            "    3️⃣ MEDIUM:",
            ...
            "```",
            "",
            "**Alternative: Use JSON-style content for structured plans**",
            "```",
            "pin_to_workspace(",
            "    label='SESSION_PLAN',",
            "    content='{",
            '        "urgent": "Check mail + reply blog comments",',
            '        "high": "Publish AI safety post",',
            '        "medium": "Research adversarial ML",',
            '        "low": "Store findings in memory",',
            '        "completed": []',
            "    }'",
            ")",
            "```",
